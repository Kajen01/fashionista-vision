...
import { computeTorsoQuad, smoothLandmarks } from '../lib/pose/poseMath';
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      poseEngine.onResults((results) => {
        ...
        setFps(Math.round(1000 / Math.max(1, now - lastFrameTime)));
        lastFrameTime = now;
        ...
        compositorRef.current.composite(
          ...
          (passCtx) => {
            ...
            const bodyQuad = computeTorsoQuad(smoothed, width, height);

            if (bodyQuad && garment.anchors) {
              rendererRef.current.drawWarpedGarment(
                img,
                garment.anchors,
                bodyQuad,
                {
                  category: garment.category || 'dress',
                  heightMultiplier: garment.heightMultiplier,
                  opacity: 0.96,
                }
              );

              if (showDebugRef.current && rendererRef.current.drawDebugAnchors) {
                rendererRef.current.drawDebugAnchors(bodyQuad);
              }
            } else if (bodyQuad) {
              const shoulderWidth = Math.abs(
                bodyQuad.upper_right.x - bodyQuad.upper_left.x
              );
              const torsoHeight = Math.abs(
                bodyQuad.lower_left.y - bodyQuad.upper_left.y
              );

              const fallbackTransform = {
                x: (bodyQuad.upper_left.x + bodyQuad.upper_right.x) / 2,
                y: (bodyQuad.upper_left.y + bodyQuad.lower_left.y) / 2,
                width: shoulderWidth * 1.2,
                height: torsoHeight * (garment.heightMultiplier || 2.0),
                rotation:
                  Math.atan2(
                    bodyQuad.upper_right.y - bodyQuad.upper_left.y,
                    bodyQuad.upper_right.x - bodyQuad.upper_left.x
                  ) *
                  (180 / Math.PI),
              };

              rendererRef.current.drawGarment(img, fallbackTransform);
            }

            rendererRef.current.ctx = ctx;
          },
          useOcclusionRef.current
            ? getOcclusionPolygons(smoothed, width, height)
            : []
        );
    ...
    <div className="pt-24 min-h-screen bg-[#fafafa] flex flex-col items-center">
      <div className="max-w-7xl w-full px-4 py-8">
        <header className="mb-12 text-center">
          ...
          <h1 className="text-5xl font-display font-black text-gray-900 mb-4 tracking-tight">
            AI Try-On Studio
          </h1>
            ...
            <div className="space-y-4 overflow-y-auto max-h-[400px] pr-2 custom-scrollbar">
              {garments.map((g) => (
                <button
                  key={g.id}
                  onClick={() =>
                    setSelectedGarment(g.id === selectedGarment?.id ? null : g)
                  }
                  className={`w-full group relative flex items-center p-4 rounded-2xl border-2 transition-all duration-300 ${
          ...
          <div className="lg:col-span-9">
            <div className="relative bg-black rounded-[3rem] overflow-hidden shadow-2xl aspect-[16/9] border-[12px] border-white ring-1 ring-gray-100">
              {isLoading && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0a0a0a] text-white z-20">
                  ...
                  <p className="mt-8 font-display tracking-[0.3em] text-xs uppercase font-bold text-white/40">
                    ...
                    <div className="relative">
                      <div
                        className={`w-2.5 h-2.5 rounded-full ${
                          isModelLoaded ? 'bg-emerald-500' : 'bg-rose-500'
                        }`}
                      />
                      <div
                        className={`absolute -inset-1 blur-sm rounded-full ${
                          isModelLoaded ? 'bg-emerald-500/50' : 'bg-rose-500/50'
                        }`}
                      />
                    </div>
                    <span>
                      {isModelLoaded ? `Neural Link: ${fps} FPS` : 'Syncing Data'}
                    </span>
                  </div>
                </div>
              )}
            ...
            <div className="mt-6 flex justify-center space-x-6">
              ...
              <button
                ...
                className={`flex items-center space-x-4 px-6 py-4 rounded-2xl transition-all shadow-sm ${
                  useOcclusion
              ...
              >
                ...
                <div
                  className={`w-10 h-6 rounded-full flex items-center px-1 transition-colors ${
                    useOcclusion ? 'bg-rose-500' : 'bg-gray-300'
                  }`}
                >
                  <div
                    className={`w-4 h-4 bg-white rounded-full transition-transform ${
                      useOcclusion ? 'translate-x-4' : ''
                    }`}
                  />
                </div>
              </button>
              ...
              >
                ...
                <div
                  className={`w-10 h-6 rounded-full flex items-center px-1 transition-colors ${
                    showDebug ? 'bg-rose-500' : 'bg-gray-300'
                  }`}
                >
                  <div
                    className={`w-4 h-4 bg-white rounded-full transition-transform ${
